...
| 運用の仕組み（2026-09-07 追加） | ✅ 利用状況の記録（usage_events）・不具合の自動通知（Gmail、同一原因6時間に1通）・日次レポート（毎朝9時JST）・ErrorBoundary。管理ハブ「利用状況」に詰まり/最後の画面/エラー |
| APNs のサーバー設定（環境変数） | ✅ 2026-09-07 20:57 キー `ShiftLogAPNs`（5HXAXSGFPB, Sandbox & Production）を Railway CLI で投入・再デプロイ |
| メール送信（Brevo） | ✅ 2026-09-07 21:28 有効化完了（Free 300通/日）。本番からの不具合通知が Gmail 受信箱に届くことを確認 |
| ビルド 1.0.0 (3)（運用の仕組み入り） | ✅ 2026-09-07 21:10 処理完了・バージョン 1.0 に紐付け |
| App 価格・コンテンツ権利の申告 | ✅ 2026-09-07 21:33 API で設定（無料 / 基準地域 JPN、第三者コンテンツなし）。未設定だと提出が 409 で止まる |
| **審査提出** | ✅ **2026-09-07 21:39 提出済み（WAITING_FOR_REVIEW）。提出物 `248b87eb…` = iOS 1.0 (3) ＋ サブスクリプショングループ「シフトログ Pro」＋ 月額・年額の2商品** |
...
Web 画面を使わずに、API と最小限のクリックで提出した。次回（1.1 以降）も同じ流れで通る。

1. `node store-assets/asc-submit.mjs check` が ✅ になるまで直す
2. 初回だけ: App の**価格**（無料 / 基準地域 JPN）と**コンテンツ権利**（第三者コンテンツなし）を設定する。未設定だと項目追加が 409（`APP_PRICING_REQUIRED` / `contentRightsDeclaration`）で止まる。スクリプトが `check` で検出する
3. **サブスクリプションは API では提出物に入れられない**（`reviewSubmissionItems` に `subscription` 関係がない）。App Store Connect の各サブスクリプション画面で「審査用に追加」→ 既存の「iOS提出物の下書き」を選ぶ。グループ自体もグループ画面で同様に追加する。初回は「新しいサブスクリプショングループはグループ内のサブスクリプションと一緒に提出」が必須
4. `node store-assets/asc-submit.mjs submit` — 下書きにアプリバージョンを追加して `submitted: true` にする。サブスクリプションと同じ提出物にまとまる
