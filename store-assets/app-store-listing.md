...
```
シフト作成、スタッフへの共有、出退勤の打刻、勤務時間の集計までをひとつのアプリで。1店舗・スタッフ5名までは無料。まずは30日間のPro無料トライアルからお試しください。
```
...
【料金】
・Free: 1店舗・スタッフ5名まで無料
・Pro: 月額980円、または年払い9,800円（2ヶ月分お得）。過去月の集計、CSV出力、給与ソフト連携、スタッフ数無制限
...
| 商品ID | `com.tomohbr.shiftlog.pro.monthly` / `com.tomohbr.shiftlog.pro.yearly` |
| 期間 | 1ヶ月 / 1年 |
| 価格 | ¥980 / ¥9,800 |
| 表示名 | シフトログ Pro |
| 説明 | 過去月の集計・CSV出力・給与ソフト連携・スタッフ数無制限が使えるプランです。 |

2商品は同じサブスクリプショングループに入れる（月額→年額のアップグレードを Apple 側で扱えるようにするため）。
